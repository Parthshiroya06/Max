...
const CollectDataScreen = () => {
  ...
  const generateImageName = (countryName, localityNumber) => {
    const year = new Date().getFullYear().toString().slice(-2); // Get last 2 digits of the year
    return `${countryName.slice(0, 3).toUpperCase()}${year}E1_L${localityNumber}_vial`;
  };
  ...
  useEffect(() => {
    if (country) {
      ...
      // Log to check if the designation is generated
...
const openCameravial = async () => {
  try {
    ...
    if (!result.didCancel && result.assets && result.assets.length > 0) {
      ...
      const autoGeneratedName = generateImageName(countryName, localityNumber);
  ...
  <View style={styles.coordinatesContainer}>
    ...
    <Text style={styles.coordinatesText}>
      ...
      {planariansPresent === null && (
        <View>
          <Text style={styles.sectionHeader}>Planarians present?</Text>
          <View style={styles.separator} />
          <View style={styles.buttonContainer}>
            <TouchableOpacity onPress={() => setPlanariansPresent('Yes')} style={styles.optionButton}>
              <Text style={styles.buttonText}>Yes</Text>
            </TouchableOpacity>
            <TouchableOpacity onPress={() => setPlanariansPresent('No')} style={styles.optionButton}>
