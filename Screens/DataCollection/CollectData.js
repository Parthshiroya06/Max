...
//   const [countryName, setCountryName] = useState(route.params?.country || "Aus");

//   const [questionAnswered, setQuestionAnswered] = useState(false);

//   const { country , noteSerial , isNewNote} = route.params;
//   //const {noteSerial} = route.params;

//   console.log ( country)
//   console.log ( noteSerial)

//   const generateImageName = (countryName, noteSerial) => {
//     const year = new Date().getFullYear().toString().slice(-2); // Get last 2 digits of the year
//     const imageName = `${countryName.slice(0, 3).toUpperCase()}${year}E1_L${noteSerial}_vial`;
...
//         const answered = await AsyncStorage.getItem(`questionAnswered_${noteSerial}`);
//         if (answered !== null) {
...
//     setQuestionAnswered(true);
//     await AsyncStorage.setItem(`questionAnswered_${noteSerial}`, answer);
//   };
...
//         ? route.params.note.localityNumber // Use the existing locality number if editing
//         : `LO${noteSerial}`; // Increment if adding a new note (assuming noteSerial is the count)

//       // Generate the locality designation
//       const generatedDesignation = `${country.slice(0, 3).toUpperCase()}${currentYear}${expedition}${localityNumber}`;
//       setLocalityDesignation(generatedDesignation);

//       // Log the generated designation
//       console.log('Locality Designation Generated:', generatedDesignation);
//     } else {
//       console.log('Country Name is not available. Locality Designation not generated.');
//     }
//   }, [country, noteSerial, route.params?.note]); // Adding route.params?.note to the dependency array to handle note changes
...
//       const autoGeneratedName = generateImageName(countryName, noteSerial);
...
//       console.log('Image captured and saved with name:', autoGeneratedName);
//     }
//   } catch (error) {
...
//     {!questionAnswered && planariansPresent === null && (
//       <View>
//         <Text style={styles.sectionHeader}>Planarians present</Text>
//         <View style={styles.separator} />
//         <View style={[styles.buttonContainer, { flexDirection: 'row', justifyContent: 'space-between' }]}>
//           <TouchableOpacity onPress={() => handleAnswer('Yes')} style={styles.optionButton}>
//             <Text style={styles.buttonText2}>Yes</Text>
//           </TouchableOpacity>
//           <TouchableOpacity onPress={() => handleAnswer('No')} style={styles.optionButton}>
...
//   const { country , noteSerial , isNewNote , projectId} = route.params;
//   //const {noteSerial} = route.params;
...
//             }
//           } else {
//             console.log('User email is not available. Cannot fetch the specific project.');
//           }
//         } else {
//           console.log('Country or projectId is not available. Locality Designation not generated.');
//         }
//       } catch (error) {
...
// Fixed the multiple picture are not saving issue
...
const CollectDataScreen = ({noteId}) => {
  ...
  const {country, noteSerial, isNewNote, projectId, noteSerial2} = route.params;
  ...
  const generateImageName = (countryName, noteSerial, expedition) => {
    const year = new Date().getFullYear().toString().slice(-2); // Get last 2 digits of the year
    const imageName = `${countryName
      .slice(0, 3)
      .toUpperCase()}${year}${expedition}_L${noteSerial}_vial`;
        ...
        if (country && projectId) {
          ...
          if (user && user.email) {
            ...
            if (projectSnapshot.exists) {
              ...
              const localityNumber = route.params?.note
              ? `LO${noteSerial2}` // Use the processed number for editing
                : `LO${noteSerial}`;
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      // Generate userName and create the new note object
      const randomUserName = `Edited by ${
  ...
  const openCameravial = async () => {
    try {
      ...
      if (!result.didCancel && result.assets && result.assets.length > 0) {
        ...
        const autoGeneratedName = generateImageName(
          countryName,
          noteSerial,
          expedition,
        );
  ...
  const openCameraHabitat = async () => {
    ...
    try {
      ...
      if (result.didCancel) {
        ...
        const expedition = `E${projectNumber}`;
        const habitatCount = imagess.length + 1; // Increment for each habitat image
        const generatedImageName = `${countryName
          .slice(0, 3)
          .toUpperCase()}${year}${expedition}_L${localityNumber}_H${habitatCount}`;
